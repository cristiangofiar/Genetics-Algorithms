...
def genera_parque():
    parque = [[0 for i in range(celdas)] for i in range(celdas)]     
    for i in range(cant_generadores_inicial):                            
        while True:                                              
            numX = np.random.randint(0, celdas)                  
            numY = np.random.randint(0, celdas)                  
            if parque[numY][numX] == 0:                          
                parque[numY][numX] = 1                           
                break                                            
...
def crossover(c1, c2):
    ...
    if c <= cr:
        ...
        cant_generadores_c1 = contar_generadores(c1)      
        if cant_generadores_c1 > cant_generadores_max:        
            c1 = corregir_parque(c1, cant_generadores_c1)      
        c2 = np.transpose(aux_columna_1 + aux_columna_2)
        cant_generadores_c2 = contar_generadores(c2)      
        if cant_generadores_c2 > cant_generadores_max:        
            c2 = corregir_parque(c2, cant_generadores_c2)      
        c1 = potencia_to_binario(c1)
        c2 = potencia_to_binario(c2)
...
def mejores_filas(cromosoma):
    ...
    cromosoma_aux = []
    for i in range(celdas):
        lista_aux.append([i, sum(cromosoma[i])])
    cant_filas = int(celdas/2)
    lista_aux = sorted(lista_aux, reverse = True, key = lambda w: w[1])[:cant_filas]
    for i in range(len(lista_aux)):
        for j in range(celdas):
            if lista_aux[i][0] == j:
                cromosoma_aux.append(cromosoma[j])
...
######## POTENCIA A BINARIO ######## 
def potencia_to_binario(cromosoma):
    for i in range(celdas):
        for j in range(celdas):
            if cromosoma[i][j] != 0:
                cromosoma[i][j] = 1
...
######## CORREGIR PARQUES ########
def corregir_parque(parque, cant_generadores_parque):
...
while True:
    ...
    ######## OBTENCION DE VIENTOS EN TIEMPO REAL ########
    # from urllib.request import urlopen
    # from pprint import pprint
    # import json
    # url = "http://api.airvisual.com/v2/nearest_city?key=424464f6-dfc8-4d3d-adba-1059fccd5a36"
    # solicitud = urlopen(url)
    # respuesta = json.loads(solicitud.read().decode("utf-8"))
    # pprint(respuesta['data']['current']['weather']['ws'])
    # input()
    ...
    cant_generadores_max = 20           # Cantidad maxima de generadores.
    cant_generadores_inicial = np.random.randint(1, cant_generadores_max + 1)
    cromosomas = []
    cromosomas_bin = []

    viento_promedio = 16
    u0 = list(viento_promedio for i in range(10))    # Velocidad del viento sin turbulencia.
    ...
    for i in range(g):
        ...
        lista_mejores_parques.append(cromosomas[f_obj.index(max(f_obj))])
    ...
    Parque = pd.ExcelWriter('C:/Users/crist/Desktop/parque.xlsx', engine='xlsxwriter')
    ...
    os.remove('C:/Users/crist/Desktop/parque.xlsx')
