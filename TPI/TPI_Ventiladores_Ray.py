...
def big_bang():
    ...
    return cromosomas_bin, cromosomas
...
def mutacion():
    ...
    for i in range(p):
        ...
        if num <= m:
            ...
            if cromosoma[corte_y][corte_x] == 0:                        #Cambio
                if contar_generadores(cromosoma) < cant_generadores_max:    #Cambio
                    cromosoma[corte_y][corte_x] = 1                     #Cambio
                    #print("Mutacion realizada. Generador agregado!")
            else:
                cromosoma[corte_y][corte_x] = 0             #Cambio
                #print("Mutacion realizada. Generador quitado...")
        lista_aux.append(cromosoma)
...
def mejor_parque():                                 # Nueva funcion!!!!!
    lista_aux = []
...
######## COPIAR PARQUE ########
def copia_parque(p):
    parque = [[0 for i in range(celdas)] for j in range(celdas)]
    for i in range(celdas):
        for j in range(celdas):
            if cromosomas_bin[p][i][j] != 0:
                parque[i][j] = 1
...
if __name__ == "__main__":
    ...
    viento_promedio = np.random.randint(7, 24)
    u0 = list(np.random.choice(np.arange(viento_promedio - 2, viento_promedio + 3), p=[0.1, 0.2, 0.4, 0.2, 0.1]) for i in range(10))    # Velocidad del viento sin turbulencia.
    ...
    cromosomas_bin, cromosomas = big_bang()
    ...
    for i in range(g):
        ...
        selec_cross()
        cromosomas_bin = mutacion()
        binario_to_potencia()

        # Quitado, no hace falta.
        # f_obj = list(np.zeros(p))
        # fitness = list(np.zeros(p))
    ...
    Datos_parque.to_excel(Tabla, sheet_name='Valores', index = False, startcol = 5, startrow = 2)

    ## DISEÑO TABLA
    workbook = Tabla.book
    worksheet = Tabla.sheets["Valores"]
    worksheet2 = Tabla.sheets["Valores"]
    ...
    worksheet2.set_column("F:O", 15, formato)
    worksheet.conditional_format("D1:D"+str(len(lista_prom)+1), {"type": "3_color_scale"})
    worksheet2.conditional_format(3, 5, 13, 15, {"type": "3_color_scale"})
    Tabla.save()
